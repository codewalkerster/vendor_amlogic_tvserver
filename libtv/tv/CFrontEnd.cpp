...
int CFrontEnd::setPara(const char *paras, bool force )
{
    ...
    case TV_FE_OFDM:
        ...
        dvbfepara.terrestrial.ofdm_mode = (fe_ofdm_mode)mFEParas.getFEMode().getGen();
        break;
    case TV_FE_DTMB:
    ...
    case TV_FE_ANALOG: {
        ...
        dvbfepara.analog.para.u.analog.std = (dvbfepara.analog.para.u.analog.std & 0xFF000000)
                                             | (mFEParas.getVFmt() & 0x00FFFFFF);
...
void CFrontEnd::dmd_fend_callback(long dev_no __unused, int event_type __unused, void *param, void *user_data)
{
    ...
    if (NULL == pFront) {
        LOGD("%s,warning : dmd_fend_callback NULL == pFront\n", __FUNCTION__);
        return ;
    }
    if (pFront->mpObserver == NULL) {
        LOGD("%s,warning : mpObserver NULL == mpObserver\n", __FUNCTION__);
        return;
    }
...
void CFrontEnd::v4l2_fend_callback(long dev_no __unused, int event_type __unused, void *param, void *user_data)
{
    ...
    CFrontEnd *pFront = (CFrontEnd *)user_data;
    if (NULL == pFront) {
        LOGD("%s,warning : v4l2_fend_callback NULL == pFront\n", __FUNCTION__);
        return ;
    }
    if (pFront->mpObserver == NULL) {
        LOGD("%s,warning : mpObserver NULL == mpObserver\n", __FUNCTION__);
...
int CFrontEnd::getInfo()
{
    ...
    AM_FEND_GetInfo(mFrontDevID, &finfo);
#endif
    //return fend_info;  notify FrontEnd message
    return 0;
}
...
int CFrontEnd::SetAnalogFrontEndSearchSlowMode(int onOff)
{
    LOGD("%s: value is %d", __FUNCTION__, onOff);
    FILE *fp = NULL;
    int ret = -1;
